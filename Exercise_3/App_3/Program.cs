...
namespace App_3
{
    ...
    class Fraction
    {
        ...
        //private static int FindNOK_(int a, int b)
        //{
        //    int num_1, num_2;
        //    if (a > b)
        //        { num_1 = a; num_2 = b; }
        ...
        public void SumResFractions(bool flag_is_sum)
        {
            ...
            if (flag_is_sum)
                sum_ch = (nok / zn_1) * ch_1 + (nok / zn_2) * ch_2;
            else
            { sum_ch = (nok / zn_1) * ch_1 - (nok / zn_2) * ch_2; act = "Разность"; sign = "-"; }

            int nod = FindNOD(Math.Abs(sum_ch), Math.Abs(nok));
            //Console.WriteLine($"\nНОК: {FindNOK(zn_1, zn_2)}");
            //Console.WriteLine($"НОД: {nod}");
            if (nod > 1)
                Console.WriteLine($"{act} дробей {ch_1}/{zn_1} {sign} {ch_2}/{zn_2} = {sum_ch}/{nok} = {(sum_ch / nod)}/{(nok / nod)}");
            else
                Console.WriteLine($"{act} дробей {ch_1}/{zn_1} {sign} {ch_2}/{zn_2} = {sum_ch}/{nok}");

        }

        public void MultFractions()
        {
            int res_ch = ch_1 * ch_2;
            int res_zn = zn_1 * zn_2;
        ...
        {
            int res_ch = ch_1 * zn_2;
            int res_zn = zn_1 * ch_2;
    ...
    class ComplexClass
    {
        ...
        {
            ComplexClass x3 = new ComplexClass();
            //Console.WriteLine($"x2.im: {x2.im}; this.im: {this.im}");
            //Console.WriteLine($"x2.re: {x2.re}; this.re: {this.re}");
            x3.im = this.im - x2.im;
            x3.re = this.re - x2.re;
            return x3;
        }
                    ...
                    switch (task_num)
                    {
                        case 1:
                            output_help.OutputTaskText("а)Дописать структуру Complex, добавив метод вычитания комплексных чисел. Продемонстрировать работу структуры;" +
                                "\nб) Дописать класс Complex, добавив методы вычитания и произведения чисел. Проверить работу класса;", 1);
                            ...
                            ComplexClass result_cl = complex_cl_1.Plus(complex_cl_2);
